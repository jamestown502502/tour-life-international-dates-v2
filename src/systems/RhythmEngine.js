...
dense_16beat:(bpm,bars)=>{const s=(60000/bpm)/4,n=[];for(let i=0;i<bars*16;i++)if(Math.random()>0.4)n.push({time:i*s,lane:i%4});return n},
polyrhythm:(bpm,bars)=>{const bm=60000/bpm,n=[];for(let b=0;b<bars*4;b++){n.push({time:b*bo,lane:b%4});if(b%3===0)n.push({time:b*bm+bm*0.33,lane:(b+2)%4})};return n.sort((a,b)=>a.time-b.time)},
breakdown:(bpm,bars)=>{const bm=60000/bpm,n=[];for(let b=0;b<bars*4;b++){if(b%4===0)n.push({time:b*bo,lane:b%4,hold:bm*1.5});if(b%4===2)n.push({time:b*bm,lane:(b+1)%4})};return n}}
...
loadSetlist(s){this.bpm=s.bpm||120;const b=s.bars||16,pf=PATTERL³S[s.notePattern]||PATTERNS.sparse_4beat;this.notes=pf(this.bpm,b);this.rewardMultiplier=s.rewardMultiplier||1.0}
...
hit(idx,t){const n=this.notes[idx];if(!n||n.hit)return null;const d=Math.abs(t-(this.startTime+n.time));let r=d<=50?'perfect':d<=100?'good':d<=160?'ok':'miss';n.hit=true;this.hits[r]++;if(r!=='miss'){this.combo++;this.maxCombo=Math.max(this.maxCombo,this.combo);this.score+=SCORE_VALUES[r]*(1+this.combo*0.1)}else{this.combo=0}this.onHitResult?.({result:r,combo:this.combo,score:this.score,noteIndex:idx});return r}
