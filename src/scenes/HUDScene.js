...
this.add.line(0,28,0,28,H,28,0xff3366,0.4)
...
_refresh(){const f=stateManager.get('fame'),fu=stateManager.get('funds'),rels=stateManager.get('relationships'),c=stateManager.getCurrentCity()
this._fameBar.width=(f/100)*80;this._fundsText.setText('$'+fu.toLocaleString());this._cityText.setText('📍 '+c.name)
const r=[rels.ryder,rels.nova,rels.jet];this._relBars.forEach((bar,i) => {bar.width=((r[i]+10)/20)*20})}}
